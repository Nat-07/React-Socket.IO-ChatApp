export default function Messages({ messagesOBJ, clientID }) {
    return (
        <div className="mt-12 mb-10 overflow-y-auto ">
            {messagesOBJ.map((message, index) => {
                const { id, name, messageText, currentTime, joined, left } =
                    message;
                // new user join
                if (joined) {
                    return (
